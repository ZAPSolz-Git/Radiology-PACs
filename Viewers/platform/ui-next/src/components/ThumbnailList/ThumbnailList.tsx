...
const ThumbnailList = ({
  ...
  return (
    <div className="flex flex-col bg-[#060a10]">
      <div
        ref={ref}
        className="flex flex-col gap-[6px] p-[6px]"
      >
        {thumbnailItems.length > 0 && (
          <div
            id="ohif-thumbnail-list"
            className="grid grid-cols-[repeat(auto-fit,_minmax(0,135px))] place-items-start gap-[6px]"
          >
        ...
        {listItems.length > 0 && (
          <div
            id="ohif-thumbnail-list"
            className="grid grid-cols-[repeat(auto-fit,_minmax(0,275px))] place-items-start gap-[4px] px-1"
          >
            {listItems.map(item => {
              const { displaySetInstanceUID, componentType, numInstances, ...rest } = item;
              const isActive = activeDisplaySetInstanceUIDs.includes(displaySetInstanceUID);
