...
function ViewportActionBar({
  ...
  return (
    ...
    >
      {getStatusComponent()}
      {!!label?.length && <span className="text-primary text-large ml-1">{label}</span>}
      ...
      <div className="border-secondary-light mx-2 border-l py-2"></div>
      <span className="text-primary mr-1 overflow-hidden text-ellipsis">{seriesDescription}</span>
      {/* Prev/Next icons */}
      <Icons.ByName
