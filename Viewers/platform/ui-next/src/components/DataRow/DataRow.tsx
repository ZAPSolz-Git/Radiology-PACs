      ...
      >
        <div
          className={cn(
            ...
            isSelected
              ? 'bg-primary/10 border border-primary/40 shadow-[0_0_10px_rgba(45,212,191,0.1)]'
              : 'bg-[#0d1320] border border-transparent hover:border-primary/20 hover:bg-[#111827]'
          )}
          onClick={onSelect}
          data-cy="data-row"
        >
          ...
          {number !== null && (
            <div
              className={cn(
                'flex h-7 max-h-7 w-7 flex-shrink-0 items-center justify-center rounded-l-lg text-[12px] font-bold',
                isSelected
                  ? 'bg-primary/25 text-primary'
                  : 'bg-[#0a0f18] text-foreground/40'
              )}
            >
              {number}
            </div>
          ...
          {colorHex && (
            <div className="flex h-7 w-5 items-center justify-center">
              <span
                className="ml-2 h-2.5 w-2.5 rounded-full ring-2 ring-primary/20"
        ...
        {details && (details.primary?.length > 0 || details.secondary?.length > 0) && (
          <div className="ml-7 border-l-2 border-primary/10 px-3 py-1.5">
            <div className="text-foreground/60 flex items-center gap-1 text-[12px] leading-normal">
              {details.primary?.length > 0 && renderDetails(details.primary)}
              {details.secondary?.length > 0 && (
                <div className="text-foreground/40 ml-auto text-[11px]">
...
const StatusInfo: React.FC<{ tooltip?: string }> = ({ tooltip }) => (
  <StatusIndicator
    tooltip={tooltip}
    icon={<Icons.Info className="h-4 w-4 text-primary" />}
    defaultTooltip="Info"
  />
);
