...
module.exports = (env, argv) => {
  ...
  const mergedConfig = merge(baseConfig, {
    ...
    devServer: {
      ...
      proxy: [
        // The active config (orthanc-local.js) uses relative '/dicom-web' roots,
        // which Orthanc serves directly on 8042.
        {
          context: ['/dicom-web'],
          target: 'http://localhost:8042',
          changeOrigin: true,
          // Orthanc has REGISTERED_USERS set, so it demands basic auth even
          // though AUTHENTICATION_ENABLED is false. Inject it here so the
          // credentials stay in the dev server and never reach the browser.
          headers: {
            Authorization:
              'Basic ' + Buffer.from('orthanc:orthanc').toString('base64'),
          },
        },
        // Kept for the upstream configs that use '/dicomweb'.
        {
          context: ['/dicomweb'],
