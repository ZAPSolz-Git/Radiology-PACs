...
async function resolveUserIdFromAeTitle(callingAeTitle) {
    ...
    try {
        const site = await Site.findOne({ scpAETitle: trimmed })
...
class RadiologyScp extends Scp {
    ...
    associationRequested(association) {
        ...
        logger.info(`C-STORE association requested from AET: ${this.callingAeTitle}`);
    ...
    trackStudy(studyUID, filePath, firstFileMetadata) {
        if (studyTracker.has(studyUID)) {
            ...
            entry.timer = setTimeout(() => this.onComplete(studyUID, entry.files, entry.callingAeTitle, entry.scpMetadata), COMPLETION_DEBOUNCE_MS);
        } else {
            const timer = setTimeout(() => {
                const entry = studyTracker.get(studyUID);
                if (entry) this.onComplete(studyUID, entry.files, entry.callingAeTitle, entry.scpMetadata);
    ...
    async onComplete(studyUID, files, callingAeTitle, scpMetadata) {
        logger.info(`Study received [SCP]: ${studyUID} — ${files.length} files — callingAeTitle: "${callingAeTitle}"`);
        ...
        try {
            await processPacsStudy(studyUID, files.length, path.join(INCOMING_DIR, studyUID), trimmedAeTitle, scpMetadata);
