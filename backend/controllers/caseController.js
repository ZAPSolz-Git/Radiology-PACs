...
export const uploadCase = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (dicomFiles.length === 0 && uploadMode !== 'pacs') {
            logger.error(`Upload failed: No files and mode is ${uploadMode}`);
            throw new AppError("No DICOM files uploaded", HTTP_STATUS.BAD_REQUEST);
        }
        ...
        if (studyInstanceUID &&
            ...
            finalStudyInstanceUID = `LOCAL_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        }

        console.log(`[Upload Debug] Resolved StudyInstanceUID: '${finalStudyInstanceUID}' (Original: '${studyInstanceUID}')`);

        console.log(`[Upload] Saving ${dicomFiles.length} DICOMs locally for Study: ${finalStudyInstanceUID}`);
        ...
        const savedDicomFiles = new Array(dicomFiles.length);
        const CONCURRENCY = 5;
        ...
        const processFile = async (file, index) => {
            ...
            savedDicomFiles[index] = metadata ? {
                ...
                seriesInstanceUID: `FALLBACK_SERIES`,
        ...
        console.log(`[Upload] All ${dicomFiles.length} files saved and streamed to Orthanc.`);

        // 2. Process Attachments in Parallel
        const savedAttachments = await Promise.all(attachmentFiles.map(async (file) => {
...
export const assignCase = asyncHandler(async (req, res) => {
    ...
    // Case 2: Assign to External Partner
        ...
        kase.addTimelineEntry(
            ...
            req.user,
            `QA ${req.user.name} assigned this case to partner "${partner.partnerName}"`,
            {
                partnerId: partnerId,
...
export const deleteCase = asyncHandler(async (req, res) => {
    ...
    if (kase.studyDirectory) {
        try {
            // Ensure we are only deleting inside the uploads folder for safety
            const safePath = path.resolve(kase.studyDirectory);
...
export const getDeleteImpact = asyncHandler(async (req, res) => {
    ...
    const caseDir = kase.studyDirectory || path.join(uploadsDir, kase.studyInstanceUID);
    let diskUsage = 0;
    try {
...
export const addAttachment = asyncHandler(async (req, res) => {
    ...
    // Determine save path
    // If case has studyDirectory, use it. Else create structure.
    let attachmentsDir;
    if (caseDoc.studyDirectory) {
        attachmentsDir = path.join(caseDoc.studyDirectory, "attachments");
    } else {
        // Fallback (shouldn't happen for valid cases)
        const studyInstanceUID = caseDoc.studyInstanceUID || `LOCAL_${Date.now()}`;
        attachmentsDir = path.join("uploads", "cases", studyInstanceUID, "attachments");
    }
