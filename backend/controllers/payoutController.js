...
export const generatePayouts = asyncHandler(async (req, res) => {
    ...
    // Find finalized cases without a payoutId where the case was finalized in that month
    ...
    // Create payout for each doctor
    for (const docId in groupedByDoctor) {
        const group = groupedByDoctor[docId];

        // Check if one already exists for this exact period to avoid duplicates (optional logic)
        // For simplicity, we just create a new one, or we can upsert. Let's create new.
