...
export const createSite = asyncHandler(async (req, res) => {
    ...
    const site = await Site.create({
        ...
        scpAETitle: scpAETitle.trim().toUpperCase(),
        scpIP: scpIP.trim(),
        scpPort: Number(scpPort),
        scuAETitle: scuAETitle.trim().toUpperCase(),
        scuIP: scuIP.trim(),
...
export const updateSite = asyncHandler(async (req, res) => {
    ...
    // Uppercase AE Titles if provided
    if (updates.scpAETitle) updates.scpAETitle = updates.scpAETitle.toUpperCase();
    if (updates.scuAETitle) updates.scuAETitle = updates.scuAETitle.toUpperCase();
 ...
 */
export const processPacsStudy = async (studyInstanceUID, expectedFileCount, directory, callingAeTitle, scpMetadata = null) => {
    ...
    logger.info(`PACS Processing started for study: ${studyInstanceUID} (Expected: ${expectedFileCount} files, callingAeTitle: "${trimmedAeTitle}")`);
    ...
    if (trimmedAeTitle) {
        logger.info(`[PACS Routing] Looking up Site with scpAETitle: "${trimmedAeTitle}"`);
        const matchedSite = await Site.findOne({ scpAETitle: trimmedAeTitle })
            .populate('assignedTechnician')
            .populate('createdBy');             // ← also populate creator
        ...
        if (matchedSite) {
            ...
            logger.warn(`[PACS Routing] ⚠️ No Site found with scpAETitle="${trimmedAeTitle}"`);
        }
    } else {
