...
export const generateInvoices = asyncHandler(async (req, res) => {
    ...
    // Date-scope to the requested month using report.finalizedAt (stable — set once at finalization).
    // Without this, every uninvoiced case from all past months would pile into the current invoice run.
    const startDate = new Date(year, month - 1, 1);
    const endDate = new Date(year, month, 1);

    // 1. Find finalized cases for this month that haven't been invoiced yet.
    //    Exclude ₹0-locked cases (no tariff matched at finalization) — they add noise but no revenue.
    const unbilledCases = await Case.find({
        status: "Finalized",
        "report.finalizedAt": { $gte: startDate, $lt: endDate },
        "billingInfo.invoiceId": { $exists: false },
        "billingInfo.total": { $gt: 0 }
    }).populate("uploadedBy");
    ...
    // cannot leave some technicians invoiced and others silently skipped.
    let session;
    try {
        session = await mongoose.startSession();
    } catch (err) {
        console.error("CRITICAL: Invoice transaction attempted but sessions are not supported on this instance.", err);
        throw new AppError("CRITICAL: System cannot guarantee invoice atomicity on this database configuration. Contact Administrator to enable Replica Sets.", HTTP_STATUS.INTERNAL_SERVER_ERROR);
    }
    ...
    try {
        ...
        console.error("[Invoice] Bulk transaction failed, rolled back:", error);
        throw error;
    } finally {
