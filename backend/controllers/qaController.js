...
export const dispatchCase = asyncHandler(async (req, res) => {
    ...
    const isReassignment = !!previousDoctorId && previousDoctorId !== doctorId.toString();

    // Detect priority change
    const previousUrgency = kase.urgency;
    const urgencyChanged = priority && priority !== previousUrgency;

    kase.assignedRadiologist = doctorId;
    kase.status = 'Assigned';
    if (priority) kase.urgency = priority;

    // Build a descriptive detail string that captures both assignment and any priority change
    let detailStr = `QA ${req.user.name} ${isReassignment ? 're-assigned' : 'assigned'} this case to Dr. ${doctor.name}`;
    if (urgencyChanged) detailStr += `. Priority changed: ${previousUrgency} → ${priority}`;

    kase.addTimelineEntry(
        isReassignment ? 'Case Re-assigned' : 'Case Assigned',
        req.user,
        detailStr,
        {
            doctorId,
            ...
            ...(isReassignment && { previousDoctorId }),
            ...(urgencyChanged && { previousUrgency, newUrgency: priority })
        }
    );
...
export const finalizeReport = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Build a procedure description string so tariff matching works against studyType values
        // like "CT Brain WO Contrast" rather than just the raw bodyPart "Brain".
        const procedureDescription = [kase.modality, kase.bodyPart].filter(Boolean).join(' ').trim();
        const matchedTariff = findMatchingTariff(tariffs, kase.modality, procedureDescription, kase.institution, kase.assignedRadiologist);
