...
export const BackgroundTasksWidget = () => {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'LOCAL_UPLOAD': return <UploadCloud className="w-5 h-5 text-amber-400" />;
      ...
      case 'PACS_PUSH': return <UploadCloud className="w-5 h-5 text-purple-400" />;
      case 'STUDY_PROCESS': return <Activity className="w-5 h-5 text-indigo-400" />;
              ...
              {activeTasks.map((task) => {
                ...
                return (
                  <li key={task.taskId} className="p-4 flex flex-col gap-2 border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors last:border-0 relative">
                    ...
                    {/* LOCAL_UPLOAD streams files from the browser; PACS_EXPORT
                        uploads DICOMs from disk to remote PACS via this browser tab.
                        Both will fail or stall if the tab is closed mid-transfer. */}
                    {(task.type === 'LOCAL_UPLOAD' || task.type === 'PACS_EXPORT') && task.status === 'RUNNING' && (
