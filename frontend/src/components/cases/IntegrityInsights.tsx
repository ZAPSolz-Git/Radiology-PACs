                                                    ...
                                                    {finding.details.problematicFrames.filter(f => f.previewUrl).map((frame, fIdx) => (
                                                        <div key={fIdx} className="relative group shrink-0">
                                                            <div className="w-24 h-24 rounded-xl overflow-hidden border border-border bg-black group-hover:border-indigo-400 transition-all cursor-zoom-in">
                                                                <img
                                                                    src={(import.meta.env.VITE_API_URL || 'http://localhost:5000/api').replace('/api', '') + frame.previewUrl}
                                                                    // src={(import.meta.env.VITE_API_URL || 'https://api.armorray.com/api').replace('/api', '') + frame.previewUrl}
                                                ...
                                                {finding.details.problematicFrames.map((frame: any, fIdx: number) => (
                                                    <DropdownMenu key={fIdx}>
                                                        <DropdownMenuTrigger asChild>
                                                            <button
                                                                className="px-3 py-1.5 rounded-xl bg-background border border-border hover:border-indigo-400 hover:text-indigo-600 hover:shadow-sm transition-all text-xs font-black whitespace-nowrap active:scale-95 flex items-center gap-2"
                                                            >
                                                                {frame.name.split('_').pop()}
