...
export function CaseChatHub({ caseId, patientName, radiologistName, technicianName, qaName, readonly = false, isSidePanel = false }: CaseChatHubProps) {
        ...
        const observer = new IntersectionObserver(
            (entries) => {
                ...
                if (hasChanges) {
                    ...
                    const messageIdsToMark = Array.from(newVisibleMessages).filter(id => {
                        const msg = messages.find(m => (m._id || m.id) === id);
                        return msg && msg.senderId !== user?._id;
                    });
                    if (messageIdsToMark.length > 0) {
                        markMessagesAsRead(messageIdsToMark);
                    }
                }
            },
    ...
    }, [messages, user, markMessagesAsRead, visibleMessages]);
        ...
        <div className={cn(
            ...
            >
                ...
                {messages.filter(msg => {
                    ...
                    if (chatType === 'qa') {
                        if (myRole === 'qa') {
                            // I'm QA, so this shouldn't happen (QA doesn't chat with QA)
                            return false;
                        }
                        // Show messages where:
                        // - I sent to QA (my role sends with chatType 'qa')
                        // - QA sent to me (QA sends with chatType matching my role)
                        return (
                    ...
                    if (chatType === 'radiologist') {
                        if (myRole === 'radiologist') {
                            // I'm Radiologist, so this shouldn't happen
                            return false;
                        }
                        // Show messages where:
                        // - I sent to Radiologist
                        // - Radiologist sent to me
                        return (
                    ...
                    if (chatType === 'technician') {
                        if (myRole === 'technician') {
                            // I'm Technician, so this shouldn't happen
                            return false;
                        }
                        // Show messages where:
                        // - I sent to Technician
                        // - Technician sent to me
                        return (
                ...
                {showScrollButton && (
                    <button
                        ...
                        className="fixed bottom-32 right-8 z-50 w-11 h-11 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full shadow-lg flex items-center justify-center transition-all"
