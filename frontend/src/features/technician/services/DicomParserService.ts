...
export const DicomParserService = {
    ...
    async parseImageFiles(files: File[]): Promise<any> {
        ...
        try {
            // Parse EXIF from the first image
            const exif = await exifr.parse(imageFiles[0]);

            return {
                institution: exif?.Make || exif?.Model || '',
                clinicalHistory: exif?.ImageDescription || exif?.UserComment || '',
                studyDate: exif?.CreateDate ? new Date(exif.CreateDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]
            };
        } catch (err) {
            console.error("[DicomParserService] Image metadata extraction failed:", err);
            return null;
        }
    },
