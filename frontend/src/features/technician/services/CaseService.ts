...
export const CaseService = {
    async getCases(filters?: any): Promise<Case[]> {
    ...
    async uploadStudy(files: File[], metadata: any, onProgress?: (progress: UploadProgress) => void): Promise<Case> {
        const BATCH_SIZE = 50;
        ...
        const createBatchData = (batchFiles: File[], isFirstBatch: boolean) => {
        ...
        const uploadBatch = async (batchFiles: File[], isFirst: boolean) => {
            const formData = createBatchData(batchFiles, isFirst);
        ...
        finalCase = await uploadBatch(firstBatchFiles, true);
        uploadedFiles = firstBatchFiles.length;
        completedBatches = 1;
        fireProgress('uploading');
        console.log(`[Frontend Upload] Case created. Uploaded ${Math.min(BATCH_SIZE, totalFiles)}/${totalFiles} files.`);
