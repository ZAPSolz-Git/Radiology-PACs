...
export function CaseDetailsModal({ isOpen, caseData, onClose, onUpdate }: CaseDetailsModalProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="w-[95vw] max-w-4xl p-0 overflow-hidden bg-background border-border shadow-2xl h-[90vh] sm:h-[550px] flex flex-col">
                <div className="flex flex-col sm:flex-row flex-1 min-h-0">
                    {/* Side Navigation - Horizontal on mobile, vertical on desktop */}
                    <div className="w-full sm:w-56 bg-muted/30 border-b sm:border-b-0 sm:border-r border-border p-2 pr-12 sm:p-6 flex flex-row sm:flex-col gap-1 sm:gap-2 shrink-0">
                        <div className="hidden sm:block text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 mb-4 px-2">Management</div>

                        <button
                            onClick={() => setActiveTab('details')}
                            className={cn(
                                "flex flex-1 sm:flex-none flex-col sm:flex-row items-center justify-center sm:justify-start gap-1 sm:gap-3 px-1 sm:px-4 py-1.5 sm:py-3 rounded-lg sm:rounded-xl transition-all text-[10px] sm:text-xs font-bold whitespace-nowrap",
                                activeTab === 'details' ? "bg-primary text-white shadow-lg shadow-primary/20" : "text-muted-foreground hover:bg-muted/50"
                            )}
                        >
                            <User className="w-4 h-4" />
                            <span className="hidden sm:inline">Patient Identity</span>
                        ...
                        >
                            ...
                            <span className="hidden sm:inline">Study Integrity</span>
                        ...
                        <div className="hidden sm:block mt-auto p-4 bg-background/50 rounded-2xl border border-border/50">
                    ...
                    <div className="flex-1 flex flex-col bg-background min-w-0 min-h-0">
                        <DialogHeader className="p-4 sm:p-6 border-b border-border flex flex-row items-center justify-between">
                            <div className="space-y-1 min-w-0">
                                <DialogTitle className="text-base sm:text-lg font-black uppercase tracking-tight truncate">
                                    <span className="hidden sm:inline">Case Details: </span>{localCaseData.patientName}
                                </DialogTitle>
                            ...
                            {(activeTab === 'details' || activeTab === 'history') && (
                                <Button
                                    ...
                                    className="h-9 px-3 sm:px-4 font-bold text-xs uppercase tracking-widest shrink-0"
                                >
                                    {isEditing ? (
                                    ...
                                    <span className="hidden sm:inline">{isEditing ? (isSubmitting ? "Saving..." : "Save Changes") : "Edit Profile"}</span>
                                    <span className="sm:hidden">{isEditing ? (isSubmitting ? "Saving" : "Save") : "Edit"}</span>
                                </Button>
                            )}
                        </DialogHeader>
                        ...
                        <div className="flex-1 flex flex-col min-h-0">
                            {localCaseData.status === 'Rejected' && (
                                <div className="m-4 sm:m-8 sm:mb-0 p-3 sm:p-4 bg-red-50 border border-red-100 rounded-xl sm:rounded-2xl flex items-start gap-3 sm:gap-4 animate-in slide-in-from-top-2">
                                    <AlertCircle className="w-4 h-4 sm:w-5 sm:h-5 text-red-600 shrink-0 mt-0.5" />
                                    <div>
                                        <h4 className="text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-red-600 mb-1">Rejection Fix Mode Active</h4>
                                        <p className="text-[10px] sm:text-[11px] text-red-700 font-medium leading-relaxed">
                                            <span className="font-black">Reason:</span> {localCaseData.rejectionReason || "No reason provided by the radiologist."}
                                        </p>
                                        <div className="mt-2 flex gap-2">
                                            <Button variant="outline" size="sm" className="h-6 sm:h-7 text-[9px] font-black uppercase text-red-600 border-red-200 bg-white hover:bg-red-50">
                                                Mark as Fixed
                                            </Button>
                                            <Button variant="ghost" size="sm" className="h-6 sm:h-7 text-[9px] font-black uppercase text-red-600 hover:bg-red-50">
                                                Appeal
                                            </Button>
                                        </div>
                                    </div>
                            ...
                            {activeTab === 'details' && (
                                <div className="flex-1 overflow-y-auto min-h-0 p-4 sm:p-8 pt-4 sm:pt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 animate-in fade-in slide-in-from-right-4">
                                    ...
                                    <div className="space-y-3 sm:space-y-4">
                                        ...
                                        <div className="p-3 sm:p-4 bg-muted/20 rounded-xl sm:rounded-2xl border border-border/50 text-center">
                                            <HistoryIcon className="w-4 h-4 sm:w-5 sm:h-5 text-muted-foreground mx-auto mb-2" />
                            ...
                            {activeTab === 'history' && (
                                <div className="flex-1 p-4 sm:p-6 flex flex-col min-h-0 animate-in fade-in slide-in-from-right-4">
                                    <HistoryManager
                            ...
                            {activeTab === 'integrity' && (
                                <div className="flex-1 overflow-y-auto min-h-0 p-4 sm:p-8 animate-in fade-in slide-in-from-right-4">
                                    <IntegrityInsights
                                        findings={localCaseData.integrityResults?.findings || []}
                            ...
                            {activeTab === 'manage_images' && (
                                <div className="flex-1 overflow-y-auto p-4 sm:p-8 min-h-0 animate-in fade-in slide-in-from-right-4">
                                    <DICOMManager
                        ...
                        <DialogFooter className="p-4 sm:p-6 bg-muted/5 border-t border-border flex flex-col-reverse sm:flex-row sm:justify-between items-center gap-3">
                            <Button variant="ghost" onClick={onClose} className="w-full sm:w-auto h-10 px-8 font-black uppercase tracking-widest text-[10px]">Close</Button>
                            <div className="flex items-center gap-2 text-red-500 bg-red-50 px-3 py-1.5 rounded-full border border-red-100">
                                <AlertCircle className="w-3.5 h-3.5 shrink-0" />
                                <span className="text-[10px] font-black uppercase tracking-tighter">Audit: System ID #1115</span>
                            </div>
                        </DialogFooter>
                    </div>
                </div>
