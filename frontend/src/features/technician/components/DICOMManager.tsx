...
export function DICOMManager({ caseId, dicomFiles, onUpdate, readOnly = false }: DICOMManagerProps) {
    ...
    const getRawUrl = (relPath: string) => {
        // Backend is on port 5000 (VITE_API_URL or defaults)
        const baseUrl = (import.meta.env.VITE_API_URL || 'http://localhost:5000/api').replace('/api', '');
        // const baseUrl = (import.meta.env.VITE_API_URL || 'https://api.armorray.com/api').replace('/api', '');
            ...
            <div className="flex flex-col gap-4 px-2">
                <div className="flex items-center justify-between">
                    <div className="space-y-1">
                        <h5 className="text-[10px] font-black uppercase text-muted-foreground italic">DICOM Sequence Manager</h5>
                        <p className="text-[9px] text-muted-foreground font-bold uppercase">Management of individual study frames • Atomic Re-indexing</p>
                    </div>
                ...
                <div className="flex items-center gap-3">
                    <div className="relative flex-1">
                        <input
                            ...
                            className="w-full pl-8 pr-4 py-1.5 text-xs font-bold bg-muted/20 border border-border rounded-lg focus:outline-none focus:border-indigo-500 transition-colors"
                        />
                    ...
                    return (
                        <div key={file.sopInstanceUID || idx} className="relative group">
                            <div className={cn(
                                ...
                                <div className="h-6 bg-muted/30 px-2 flex items-center justify-between border-b border-border/50">
                                    <span className="text-[8px] font-black text-muted-foreground uppercase">#{file.instanceNumber || idx + 1}</span>
            ...
            <div className="p-4 rounded-2xl bg-muted/20 border border-border/50 flex items-start gap-3">
                ...
                <div className="space-y-1">
                    ...
                    <p className="text-[9px] text-muted-foreground font-medium leading-relaxed">
                        Adding or deleting frames will trigger an automatic <span className="text-indigo-600 font-bold">AI Consistency Audit</span>.
                        Correcting gaps (e.g. adding a missing frame #9) improves the study's clinical grade.
                    </p>
                </div>
            </div>
        </div>
