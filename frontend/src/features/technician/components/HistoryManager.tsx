...
return (
        <>
            ...
            <div className="flex bg-muted/30 border-b border-border p-1 overflow-x-auto">
                {(['templates', 'edit', 'files'] as const).map(tab => (
                    <button
                        key={tab}
                        onClick={() => setSelectedTab(tab)}
                        className={cn(
                            "flex items-center justify-center gap-1.5 sm:gap-2 py-2 px-2 sm:px-3 text-xs font-bold transition-all rounded-md capitalize whitespace-nowrap shrink-0",
                            selectedTab === tab ? "bg-background shadow-sm text-primary border border-border/50" : "text-muted-foreground hover:text-foreground"
                        )}
                    >
                        {tab === 'templates' && <Zap className="w-3 h-3 sm:w-3.5 sm:h-3.5" />}
                        {tab === 'edit' && <ScrollText className="w-3 h-3 sm:w-3.5 sm:h-3.5" />}
                        {tab === 'files' && <FileUp className="w-3 h-3 sm:w-3.5 sm:h-3.5" />}
                        <span className="hidden sm:inline">{tab}</span>
                        <span className="sm:hidden">{tab === 'templates' ? 'Temp' : tab === 'edit' ? 'Edit' : 'Files'}</span>
                        {tab === 'files' && (attachedFiles.length + existingAttachments.length) > 0 && (
                            <span className="bg-primary text-white text-[9px] min-w-[16px] h-4 rounded-full flex items-center justify-center ml-1">
            ...
            <div className="flex-1 overflow-y-auto p-2 sm:p-4 min-h-[200px]">
                            ...
                            {HISTORY_TEMPLATES.map((t) => (
                                ...
                                >
                                    <div className="flex items-center justify-between mb-1 gap-2">
                                        <span className="text-sm font-bold group-hover:text-primary truncate">{t.title}</span>
                                        <Badge variant="outline" className="text-[9px] h-4 uppercase tracking-tighter opacity-70 shrink-0">
                                            {t.modality}
                                        </Badge>
                                    </div>
                                    <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed italic break-words">"{t.content}"</p>
                                </button>
                            ))}
                        </div>
                    </div>
                                            ...
                                            {existingAttachments.map((file, i) => (
                                                <div key={`existing-${i}`} className="flex items-center justify-between p-2 rounded-lg bg-muted/30 border border-border text-xs group">
                                                    <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                                                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-600 shrink-0">
                                                    ...
                                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                                                        <Button
                                                            variant="ghost"
                                                            size="icon"
                                                            className="h-6 w-6 sm:h-7 sm:w-7 text-indigo-500 hover:bg-indigo-50/30"
                                                        ...
                                                        {onRemoveExistingAttachment && (
                                                            <Button
                                                                variant="ghost"
                                                                size="icon"
                                                                className="h-6 w-6 sm:h-7 sm:w-7 text-red-500 hover:bg-red-50/30"
                                                                onClick={() => {
                                        ...
                                        <div className="space-y-1">
                                            {attachedFiles.map((file, i) => (
                                                <div key={i} className="flex items-center justify-between p-2 rounded-lg bg-muted/30 border border-border text-xs group">
                                                    <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                                                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded bg-green-500/10 border border-green-500/20 flex items-center justify-center text-green-600 shrink-0">
                                                            <FileUp className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                                                        </div>
                                                        <div className="overflow-hidden min-w-0">
                                                            <div className="font-bold truncate" title={file.name}>{file.name}</div>
                                                    ...
                                                    <Button
                                                        variant="ghost"
                                                        size="icon"
                                                        className="h-6 w-6 sm:h-7 sm:w-7 text-red-500 opacity-0 group-hover:opacity-100 transition-opacity shrink-0"
