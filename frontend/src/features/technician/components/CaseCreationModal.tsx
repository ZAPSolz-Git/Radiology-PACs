...
export function CaseCreationModal({ isOpen, onClose, onSubmit, editCase, onEnrich }: CaseCreationModalProps) {
    ...
    // PACS States
    const [uploadMode, setUploadMode] = useState<'folder' | 'pacs' | 'pacs-export' | 'pacs-received' | 'image-upload'>('folder');
    ...
    const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setIsParsing(true);
        try {
            // Use the service to extract EXIF from the first image to pre-fill
            const prefill = await DicomParserService.parseImageFiles(files);
            if (prefill) {
                setPatientData(prev => ({
                    ...prev,
                    institution: prefill.institution || prev.institution,
                    // Note: Date parsing might need adjustment based on format
                    // patientName, patientId, etc remain empty as per requirements
                }));
                if (prefill.clinicalHistory) {
                    setClinicalHistory(prefill.clinicalHistory);
                }
            }
            toast.success(`Staged ${files.length} clinical images`);
        } catch (err) {
    ...
    const handleFinalSubmit = async () => {
        ...
        const effectiveStudyUID = isValidUID(metadata?.studyInstanceUID)
            ...
            : (isValidUID(editCase?.studyInstanceUID) ? editCase!.studyInstanceUID : `1.2.826.0.1.3680043.8.498.${Date.now()}.${Math.floor(Math.random() * 10000)}`);
            ...
            if (editCase) {
                ...
                CaseService.uploadStudy(selectedFiles, {
                    ...
                    setLocalTask({
                        ...
                        type: 'LOCAL_UPLOAD',
                        status: 'COMPLETED',
                        progress: selectedFiles.length + imageFiles.length,
                        total: selectedFiles.length + imageFiles.length,
                        message: 'Upload Complete',
                        metadata: { patientName: finalData.patientName }
                    });
                    // Toast only fires on confirmed success — not before the upload starts
                    ...
                    setLocalTask({
                        ...
                        progress: 0,
                        total: selectedFiles.length,
                        message: 'Upload Failed — queued offline',
                        metadata: { patientName: finalData.patientName }
                    });
                    try {
                        await OfflineSyncService.queueStudy(finalData, selectedFiles);
                        toast.warning("Upload failed. Study saved to offline queue and will retry when reconnected.");
                    } catch {
                        toast.error("Upload failed and offline queue unavailable. Please retry manually.");
                    }
    ...
    const canGoNext = () => {
        if (step === 1) {
            ...
            if (uploadMode === 'image-upload') return imageFiles.length > 0;
                        ...
                        <div className="flex-1 overflow-y-auto p-4 sm:p-6 italic:not-italic">
                            {step === 1 && (
                                <div className="h-full flex flex-col">
                                    {/* Mode Selector */}
                                    <div className="flex flex-wrap bg-muted/20 p-1 rounded-xl sm:rounded-2xl border border-border w-full sm:w-fit mb-4 sm:mb-8 mx-auto gap-1">
                                        <button
                                            onClick={() => setUploadMode('folder')}
                                            className={cn(
                                                "flex items-center justify-center gap-2 px-3 sm:px-5 py-2 sm:py-2 text-[9px] sm:text-[10px] font-black uppercase tracking-widest rounded-lg sm:rounded-xl transition-all",
                                        ...
                                        <button
                                            onClick={() => setUploadMode('image-upload')}
                                            className={cn(
                                                "flex items-center justify-center gap-2 px-3 sm:px-5 py-2 sm:py-2 text-[9px] sm:text-[10px] font-black uppercase tracking-widest rounded-lg sm:rounded-xl transition-all",
                                                uploadMode === 'image-upload' ? "bg-background shadow-lg text-amber-600 border border-border/50" : "text-muted-foreground hover:text-foreground"
                                            )}
                                        >
                                    ...
                                    ) : uploadMode === 'image-upload' ? (
                                        <div className="flex-1 flex flex-col items-center justify-center">
                                            {isParsing ? (
                                                <div className="text-center space-y-4">
                                                    <Loader2 className="w-12 h-12 animate-spin text-amber-500 mx-auto" />
                                                    <div className="space-y-1">
                                                        <h3 className="font-bold uppercase tracking-wider text-sm text-foreground">Analyzing Clinical Images</h3>
                                                        <p className="text-[11px] text-muted-foreground italic font-medium">Extracting EXIF data and timestamps...</p>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div className="w-full max-w-4xl space-y-6">
                                                    ...
                                                    {imageFiles.length === 0 ? (
                                                        <div
                                                            ...
                                                            className="group relative h-64 flex flex-col items-center justify-center text-center p-8 border-2 border-dashed border-border rounded-3xl bg-amber-500/5 hover:border-amber-500/50 hover:bg-amber-500/10 transition-all cursor-pointer"
                                                        >
                                                            ...
                                                            <h3 className="text-sm font-black uppercase tracking-widest mb-2 text-foreground">Upload Clinical Images</h3>
                                                            <p className="text-[11px] text-muted-foreground px-4 leading-relaxed font-medium max-w-xs">
                                                                Select JPG or PNG clinical images. They will be converted to DICOM format.
                                                            </p>
                                                        </div>
                                                    ) : (
                                                        <div className="space-y-4 animate-in fade-in zoom-in-95 duration-300">
                                                            <div className="flex items-center justify-between px-2">
                                                                <div>
                                                                    <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Staged for Dicomization</h4>
                                                                    <p className="text-[12px] font-bold text-foreground">{imageFiles.length} Images Selected</p>
                                                                </div>
                                                                <Button
                                                                    variant="outline"
                                                                    size="sm"
                                                            ...
                                                            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 max-h-[350px] overflow-y-auto p-2 custom-scrollbar border border-border/50 rounded-2xl bg-muted/5">
                                                                {imagePreviews.map((url, i) => (
                                                                    <div key={i} className="group relative aspect-square rounded-xl overflow-hidden border border-border bg-background shadow-sm hover:shadow-md transition-all">
                                                                        ...
                                                                        <div className="absolute bottom-0 left-0 right-0 p-1 bg-black/60 text-white text-[8px] font-medium truncate">
                                                                            {imageFiles[i].name}
                                                                        </div>
                                                                    </div>
                                                                ))}
