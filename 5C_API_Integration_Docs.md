...
### 3.1 Fetch All Assigned Cases
Fetches a list of all cases currently assigned to your organization (5C Network) for reporting or QA.

- **URL:** `/api/v1/external/cases`
- **Method:** `GET`
- **Required Scope:** `read:cases`

**Query Parameters (Optional):**
- `status` (String): Filter cases by their current status (e.g., `Assigned`, `QA_Audit`).
...
**Example Request:**
```bash
curl -X GET "https://api.armorray.com/api/v1/external/cases?status=Assigned" \
...
### 3.3 Fetch Specific Case Detail
...
### 3.4 Sync Finalized Report (Upload)
